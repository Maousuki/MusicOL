  ...
  <div class="container">
    ...
    <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="资源更新" width="120" align="center">
        <template v-slot="scope">
          <el-upload :headers="myHeader" :action="updateSongImg(scope.row.id)" :show-file-list="false"
            :on-success="handleImgSuccess" :before-upload="beforeImgUpload">
            <el-button>更新图片</el-button>
          </el-upload>
          <br />
          <el-upload :headers="myHeader" :action="updateSongUrl(scope.row.id)" :show-file-list="false"
...
const { routerManager, beforeImgUpload, beforeSongUpload, parseLyric } = utils();
const store = useStore();
const route = useRoute()
const attachImageUrl = HttpManager.attachImageUrl
const myHeader = computed(() => { return { token: store.getters.token } })
...
singerName.value = route.query.name
...
async function getData() {
  tableData.value = [];
  tempDate.value = [];
  const result = (await HttpManager.getSongOfSingerId(singerId.value))
  tableData.value = result.data;
  tempDate.value = result.data;
  currentPage.value = 1;
}
function setSongUrl(row) {
  store.commit("setSongUrl", row.url);
...
function handleSongSuccess(res) {
  ...
  if (res.message === "SUCCESS") getData();
}
// 更新图片
...
function goCommentPage(id) {
  const breadcrumbList = reactive([
    {
      path: RouterName.ManageSinger,
      name: "歌手管理",
    },
    {
      path: RouterName.ManageSong,
      query: {
        id: singerId.value,
  ...
  routerManager(RouterName.ManageComment, { path: RouterName.ManageComment, query: { id, type: 0 } });
}
...
function addSong() {
  ...
  req.open("post", HttpManager.attachImageUrl(`/song/add`), true);
  req.setRequestHeader("token", store.getters.token);
...
async function saveEdit() {
  ...
  const result = (await HttpManager.updateSongMsg(JSON.stringify(data), { headers: { 'token': store.getters.token } }))
  ElMessage({
...
async function confirm() {
  const result = (await HttpManager.deleteSong(idx.value, { headers: { 'token': store.getters.token } }))
  ElMessage({
    message: result.message,
    type: result.type,
  })
