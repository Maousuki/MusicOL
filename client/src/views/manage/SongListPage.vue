    ...
    <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="歌单图片" width="110" align="center">
        <template v-slot="scope">
          ...
          <el-upload :headers="myHeader" :action="uploadUrl(scope.row.id)" :show-file-list="false"
...
const { routerManager, beforeImgUpload } = utils();
...
function uploadUrl(id) {
  return HttpManager.attachImageUrl(`/songList/updatePic?id=${id}`);
}
// 更新图片
function handleImgSuccess(response, file) {
  ...
  if (response.message === "SUCCESS") getData();
}
...
function goContentPage(id) {
  const breadcrumbList = reactive([
    {
      path: RouterName.ManageSongList,
      name: "歌单管理",
    },
    {
      path: "",
  ...
  routerManager(RouterName.ManageListSong, { path: RouterName.ManageListSong, query: { id } });
}
function goCommentPage(id) {
  ...
  store.commit("setBreadcrumbList", breadcrumbList);
  routerManager(RouterName.ManageComment, { path: RouterName.ManageComment, query: { id, type: 1 } });
}
...
async function addsongList() {
  const data = {
    ...
    style: registerForm.style
  }

  const result = (await HttpManager.setSongList(JSON.stringify(data), { headers: { 'token': store.getters.token } }))
  ElMessage({
...
async function saveEdit() {
  ...
  const result = (await HttpManager.updateSongListMsg(JSON.stringify(data), { headers: { 'token': store.getters.token } }))
  ElMessage({
    message: result.message,
    type: result.type,
  })
...
async function confirm() {
  const result = await HttpManager.deleteSongList(idx.value, { headers: { 'token': store.getters.token } })
  ElMessage({
    message: result.message,
    type: result.type,
  })
