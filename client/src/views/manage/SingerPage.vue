...
const { changeSex, routerManager, beforeImgUpload, getBirth } = utils();
...
function uploadUrl(id) {
  return HttpManager.attachImageUrl(`/singer/avatar?id=${id}`);
}
...
function goSongPage(row) {
  ...
  routerManager(RouterName.ManageSong, {
...
async function addsinger() {
  ...
  const result = (await HttpManager.setSinger(JSON.stringify(data), { headers: { 'token': store.getters.token } }))
  ElMessage({
...
async function saveEdit() {
  try {
    ...
    const result = (await HttpManager.updateSingerMsg(JSON.stringify(data), { headers: { 'token': store.getters.token } }))
    ElMessage({
    message: result.message,
    type: result.type,
  })
...
async function confirm() {
  const result = (await HttpManager.deleteSinger(idx.value, { headers: { 'token': store.getters.token } }))
  ElMessage({
    message: result.message,
    type: result.type,
  })
